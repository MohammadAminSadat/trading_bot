...
ExponentialRSI::ExponentialRSI(std::size_t period) : period_{period} {
  if (period == 0) {
    throw std::invalid_argument("Period must not equal to 0");
  }
  id_ = 6; // TODO : place holder fo now
  alpha_ = 2.0 / (1.0 + static_cast<double>(period));
}

std::optional<IndicatorResult> ExponentialRSI::update(const MarketData::Candle &candle) {
...
std::optional<SignalOutput> ExponentialRSI::update(double value) noexcept {
  if (!previous_value_) {
